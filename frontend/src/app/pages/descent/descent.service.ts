...
export class DescentService {
  ...
  constructor( private http: HttpClient,
    ...
    create(descent: Descent): Observable <Descent> {
      return this.http.post<Descent>(this.baseUrl, descent, {headers: this.headers}).pipe(
    ...
    update(descentUpdate: Descent): Observable<Descent> {
      const url = `${this.baseUrl}/${descentUpdate.id_descentralizacao}`;
      return this.http.put<Descent>(url, descentUpdate, {headers: this.headers}).pipe(
        map(obj => obj), catchError( e => this.errorHandler(e))
      );
    }
